...
// Ported from Kobra LAN Monitor's LanCredentials.cs -- same printer, same reverse-engineered
// handshake. Only the printer's LAN IP is ever needed from the user; everything else
// (broker address, MQTT username/password, client cert) is self-discovered here.
...
public static class LanCredentialDiscovery
{
    ...
    public static async Task<LanCredentials> DiscoverAsync(string printerHost, CancellationToken ct)
    {
        ...
        var deviceId = "KobraTimeLapse-" + RandomAlphaNumeric(8);
